...
def _(LinearRegression, analysis, anomaly_z, mean_absolute_percentage_error, np, pd, r2_score):
    ...
    modeled_parts = []
    plant_models = {}
    for plant_name, plant_data in analysis.groupby("plant"):
        plant_out, plant_model = fit_plant_model(plant_data)
        modeled_parts.append(plant_out)
        plant_models[plant_name] = plant_model
...
def _(analysis, mo, plt, results):
    energy_figs = []
    for plant_name, plant_data in results.groupby("plant"):
        d = plant_data.sort_values("start_date")
        fig, ax = plt.subplots(figsize=(9, 4.5))
        ax.plot(d["start_date"], d["plant_kwh"], marker="o", label="Actual kWh")
        ax.plot(d["start_date"], d["expected_kwh"], marker="o", label="Weather-normalized expected kWh")
        ax.set_title(f"{plant_name} Plant - Actual vs Expected")
        ax.set_xlabel("Billing period")
        ax.set_ylabel("kWh")
        ax.legend()
        ax.grid(alpha=0.25)
        fig.tight_layout()
        energy_figs.append(mo.vstack([mo.md(f"### {plant_name} Plant"), fig]))

    cdd_fig, cdd_ax = plt.subplots(figsize=(9, 4.5))
    for plant_name, plant_data in analysis.groupby("plant"):
        d = plant_data.sort_values("start_date")
        cdd_ax.plot(d["start_date"], d["kwh_per_cdd"], marker="o", label=plant_name)
...
def _(mo, pca_loadings, pca_scores, pca_summary, pd, plt, results):
    if pca_scores.empty or "PC2" not in pca_scores.columns:
        ...
        pca_figs = []
        for plant_name, plant_data in pca_results.dropna(subset=["PC1", "PC2", "residual_pct"]).groupby("plant"):
            d = plant_data.sort_values("start_date").copy()
            fig, ax = plt.subplots(figsize=(8.5, 5.0))
            sizes = 35 + 3 * d["residual_pct"].abs().clip(upper=60)
            sc = ax.scatter(d["PC1"], d["PC2"], s=sizes, c=d["residual_pct"], alpha=0.75)
            ax.axhline(0, linewidth=0.8, alpha=0.4)
            ax.axvline(0, linewidth=0.8, alpha=0.4)
            ax.set_title(f"{plant_name} Plant - PCA Weather Regimes vs Energy Residual")
            ax.set_xlabel(f"PC1 ({pca_summary.loc[0, 'Explained_Variance_pct']:.1f}% variance)")
            ax.set_ylabel(f"PC2 ({pca_summary.loc[1, 'Explained_Variance_pct']:.1f}% variance)")
            colorbar = fig.colorbar(sc, ax=ax)
            colorbar.set_label("Energy residual (%)")
            ax.grid(alpha=0.2)
            labels = pd.concat([
                d.nlargest(min(5, len(d)), "residual_pct", keep="all"),
                d.nsmallest(min(3, len(d)), "residual_pct", keep="all"),
            ]).drop_duplicates(subset=["start_date"])
            for _, row in labels.iterrows():
                ax.annotate(
                    pd.to_datetime(row["end_date"]).strftime("%b %Y"),
                    (row["PC1"], row["PC2"]),
                    xytext=(4, 4), textcoords="offset points", fontsize=8,
                )
            fig.tight_layout()
            pca_figs.append(mo.vstack([mo.md(f"### {plant_name}"), fig]))
...
@app.cell
def _(io, pd, utility, meter_period, meter_pivot, plant_period, weather, results, diagnostics, flagged, chillers, cost_summary, pca_scores, pca_loadings, pca_summary, meter_summary):
    ...
    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        ...
        if not pca_scores.empty:
            pca_scores.to_excel(writer, sheet_name="PCA Scores", index=False)
...
def _(
    ...
    for plant_name, group in valid_results.groupby("plant"):
        high_count = int((group["flag"] == "HIGH USE").sum())
        report_doc.add_paragraph(f"{plant_name} had {high_count} billing period(s) flagged as unusually high use.")
    for _, row in cost_summary.iterrows():
        report_doc.add_paragraph(
            f"{row['plant']} positive-residual energy represents approximately ${row['estimated_excess_energy_cost']:,.0f} at the configured blended rate. "
